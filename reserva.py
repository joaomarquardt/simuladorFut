...
timeres = ['AMÉRICA-MG', [65, 71, 75, 63], 'ATHLETICO-PR', [72, 79, 79, 75], 'ATLÉTICO-GO', [69, 66, 65, 65],
    ...
    if opcao == 1:
        ...
        while True:
            ...
            if opcao == 1:
                ...
                print(f'\033[1;4m{"NOME DA EQUIPE":<15}', '|  CLASSIFICAÇÃO GERAL\033[m')
                melhorAtaque = melhorDefesa = 0
                nomeAtaque = nomeDefesa = ''
                for c in list:
                    if c == 0:
                        nomeAtaque = c.getNome()
                        nomeDefesa = c.getNome()
                        melhorAtaque = c.getAtaque()
                        melhorDefesa = c.getDefesa()
                    else:
                        if c.getAtaque() > melhorAtaque:
                            melhorAtaque = c.getAtaque()
                            nomeAtaque = c.getNome()
                        if c.getDefesa() > melhorDefesa:
                            melhorDefesa = c.getDefesa()
                            nomeDefesa = c.getNome()

                    print(f'{c.getNome():<15}', f'|  {c.getPontos():}')
                    if c.getNome() == 'PALMEIRAS':
                        print(f'Ataque: {c.getAtaque()} | Defesa: {c.getDefesa()} | Casa: {c.getCasa()} | Fora: {c.getFora()} ')
                print(f'MELHOR ATAQUE: {melhorAtaque}, do {nomeAtaque} | MELHOR DEFESA: {melhorDefesa}, do {nomeDefesa}')
            else:
                break
